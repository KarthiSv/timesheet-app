...
function parseClarityFile(wb) {
  ...
  var byName = {};
  rows.forEach(function(row) {
    var name = String(getCol(row, ["resource name"]) || "").trim();
    if (!name) return;
    var key = normalizeName(name);
    if (!byName[key]) {
      byName[key] = {
        rawName: name, normalizedName: key,
        resourceManager: String(getCol(row, ["resource manager"]) || ""),
        transactionClass: String(getCol(row, ["transaction class"]) || ""),
        timesheetStatus: String(getCol(row, ["timesheet status"]) || ""),
        approvedBy: String(getCol(row, ["approved by"]) || ""),
        resourceActive: String(getCol(row, ["resource active"]) || ""),
        actualHours: 0, periods: [],
        monthlyHours: {}, // { "January-2026": hours }
      };
    }
    var hrs = parseFloat(getCol(row, ["total actual hours", "actual hours"]) || 0) || 0;
    byName[key].actualHours += hrs;
    var period = String(getCol(row, ["time reporting period", "period"]) || "").trim();
    if (period && byName[key].periods.indexOf(period) === -1) byName[key].periods.push(period);
    // Store hours by month key
    // Try to extract month-year from the Time Reporting Period value of this row
    var periodStr = String(getCol(row, ["time reporting period","period"]) || "").trim();
    var rowMonthKey = null;
    if (periodStr) {
      // Try "MM/DD/YYYY" or "MM/DD/YYYY - MM/DD/YYYY" format
      var dateMatch = periodStr.match(/(\d{1,2})[\/\-](\d{1,2})[\/\-](\d{4})/);
      if (dateMatch) {
        var pMonth = parseInt(dateMatch[1]);
        var pYear  = parseInt(dateMatch[3]);
        var MFULL_P = ["January","February","March","April","May","June","July","August","September","October","November","December"];
        if (pMonth >= 1 && pMonth <= 12) rowMonthKey = MFULL_P[pMonth-1] + "-" + pYear;
      }
      // Try "Month YYYY" or "Month-YYYY" format
      if (!rowMonthKey) {
        var MFULL_P2 = ["January","February","March","April","May","June","July","August","September","October","November","December"];
        ...
        if (yearM) {
          ...
          for (var mi2 = 0; mi2 < MFULL_P2.length; mi2++) {
            if (pl.indexOf(MFULL_P2[mi2].toLowerCase()) !== -1 || pl.indexOf(MABBR_P[mi2].toLowerCase()) !== -1) {
              rowMonthKey = MFULL_P2[mi2] + "-" + py; break;
            }
          }
        }
      }
    }
    var mkey = rowMonthKey || (monthInfo ? monthInfo.key : "Unknown");
    byName[key].monthlyHours[mkey] = (byName[key].monthlyHours[mkey] || 0) + hrs;
    var st  = String(getCol(row, ["timesheet status"]) || "").trim();
    if (st)  byName[key].timesheetStatus = st;
    var mgr = String(getCol(row, ["resource manager"]) || "").trim();
    if (mgr) byName[key].resourceManager = mgr;
    var appr = String(getCol(row, ["approved by"]) || "").trim();
    if (appr) byName[key].approvedBy = appr;
    var active = String(getCol(row, ["resource active"]) || "").trim();
    if (active) byName[key].resourceActive = active;
  });
  return {
    sheetName: sheetName,
